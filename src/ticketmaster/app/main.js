...
function getPythonPath() {
  ...
  return path.join(__dirname, '..', 'backend', 'task.py');
}
...
function createWindow() {
  ...
  mainWindow.on('close', (e) => {
    ...
    if (pythonProcess) {
      ...
      tray.displayBalloon?.({
        title: 'TicketWatch',
        content: 'Still watching in the background. Right-click tray icon to quit.',
      });
    } else {
      app.isQuiting = true;
      app.quit();
...
function startPython(taskConfig) {
  ...
  pythonProcess.on('close', (code) => {
    mainWindow?.webContents.send('python-message', { type: 'stopped', code });
    pythonProcess = null;
  });
}
